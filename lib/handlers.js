...
try {
  l.less = require('less');
    ...
    console.log("WARNING: 'less' could not be required.");
    console.log("         You won't be able to parse .less files.");
    console.log("         Install it with `npm install less`.");
}
  ...
  that.handle = function(file, request, callback) {
    that.next.handle(file, request, function(response) {
      if (/sc_super\(\s*[^\)\s]+\s*\)/.test(response.data)) {
          console.log('ERROR in ' + file.path + ': sc_super() should not be called with arguments. Modify the arguments array instead.');
      }
    ...
    that.next.handle(file, request, function(response) {
      ...
      response.data = response.data.toString('utf8').gsub(re, function(match) {
        ...
        if (!file.framework.server.files[path]) {
          ...
          if (!file.framework.server.files[path]) {
              console.log('WARNING: ' + path + ' referenced in ' + file.path + ' but was not found.');
          }
        }
    ...
    that.next.handle(file, request, function(response) {
      ...
      if (!l.jslint(response.data.toString(), options)) {
        for (i = 0; i < l.jslint.errors.length; ++i) {
          e = l.jslint.errors[i];
          if (e) {
              console.log('WARNING: jslint error at line ' + e.line + ' character ' + e.character + ': ' + e.reason);
